...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        updateFactorList = []
        # join and eliminate hidden variables
        for variable in eliminationOrder:
            ...
            for factor in factorList:
                if variable in factor.variables():
                    ...
                    updateFactorList += [factor]
            newFactor = joinFactorsByVariable(containsV, variable)[1]
            if len(newFactor.unconditionedVariables()) == 1:
                factorList = updateFactorList
            else:
                removedVarFactor = eliminate(newFactor, variable)
                updateFactorList += [removedVarFactor]
                factorList = updateFactorList
            updateFactorList = []
        #join rest
        newFactor = joinFactors(factorList)
        #normalize
        normalized = normalize(newFactor)
        return normalized
