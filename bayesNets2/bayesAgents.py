...
def constructBayesNet(gameState):
    ...
    obsVars += [X_POS_VAR] + [Y_POS_VAR] + [FOOD_HOUSE_VAR] + [GHOST_HOUSE_VAR]
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVars += [obsVar]
    #Generate specific Bayes net in problem
...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    def position(corner):
        if corner == TOP_LEFT_VAL:
            return topLeftPos
        elif corner ==BOTTOM_LEFT_VAL:
            return bottomLeftPos
        elif corner == TOP_RIGHT_VAL:
            return topRightPos
        else:
            return bottomRightPos
    def adjacent(position, center):
        return position in gameState.getHouseWalls(center)
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            currFactor = bn.Factor([obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for assignment in currFactor.getAllPossibleAssignmentDicts():
                ghostHouseVal = position(assignment[GHOST_HOUSE_VAR])
                foodHouseVal = position(assignment[FOOD_HOUSE_VAR])
                color = assignment[obsVar]
                prob = 0
                if not adjacent(obsPos, ghostHouseVal) and not adjacent(obsPos, foodHouseVal):
                    if color == RED_OBS_VAL:
                        ...
                        prob = 1
                if adjacent(obsPos, ghostHouseVal):
                    if color == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif color == BLUE_OBS_VAL:
                        ...
                        prob = 0
                if adjacent(obsPos, foodHouseVal):
                    if color == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif color == BLUE_OBS_VAL:
